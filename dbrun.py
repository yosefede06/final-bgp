...
file_queue = queue.Queue(maxsize=5)  # Ensure only 10 files are queued for processing
...
def process_mrt_file():
    ...
    while True:
        ...
        with open(output_csv_max, 'a', newline='') as csvfile_max:
            ...
            max_counts.append(max_processed_data)
            os.remove(mrt_file)
        end_time = time.time()
...
class ProcessingStrategies:
    ...
    def max_paths(cursor):
        cursor.execute('''
            SELECT MAX(path_count)
            FROM (
                SELECT COUNT(DISTINCT as_path) AS path_count
                FROM bgp_paths
                GROUP BY peer_as, prefix
            )
        ''')
        max_path = cursor.fetchone()
        return max_path[0] if max_path else 0
