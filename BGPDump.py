...
class Prefix:
    ...
    def formatted_address(self):
        return socket.inet_ntoa(struct.pack("!I", self.address))
...
class TableDumpV1:
    ...
    def sourceAS(self):
        ...
        return self.body.source_as

    @property
    def announce(self):
        return [(f"{inet_ntop(AF_INET, self.bgp.ffi.buffer(self.bgp.ffi.addressof(self.body.announce[i].address.v4_addr))[:])}"
                 f"/{self.body.announce[i].len}") for i in range(self.body.announce_count)]

    @property
    def withdraw(self):
        return [(
                    f"{inet_ntop(AF_INET, self.bgp.ffi.buffer(self.bgp.ffi.addressof(self.body.withdraw[i].address.v4_addr))[:])}"
                    f"/{self.body.withdraw[i].len}") for i in range(self.body.withdraw_count)]

    @property
    def withdraw_count(self):
        return self.body.withdraw_count
...
class BGPEntry:
    ...
    def body(self):
        ...
        if self.entry.type == CConst.BGPDUMP_TYPE_ZEBRA_BGP:
            self._body = TableDumpV1(self.bgp, self.entry.subtype, self.entry.body.zebra_message)
